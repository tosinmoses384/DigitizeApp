...
/**
 * Simple deep link handler for user URLs (e.g., digitize-app://user/123)
 * 
 * Simplified version to prevent infinite loops while maintaining functionality:
 * - If user is authenticated: navigates directly to SellerProfile with Redux state
...
export default function UserDeepLinkHandler() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      console.log('✅ User authenticated, navigating to user profile:', userId);
      dispatch(setSellerId(userId));
      router.replace('/SellerProfile?fromDeepLink=true' as any);
    } else {
