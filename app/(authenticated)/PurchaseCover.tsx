...
const PurchaseCover = () => {
  const { t } = useI18n();
  const pageTitle = t('about.purchaseCover') || t('aboutPages.purchaseCover') || "Purchase Cover";

  const baseUrl = process.env.EXPO_PUBLIC_WEB_BASE_URL || 'https://staging.digitizeapp.com';
  const webViewUrl = `${baseUrl}/purchase-cover?layout=none`;
