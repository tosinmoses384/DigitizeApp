...
const OrderCompletedActivity = ({
  message,
  profileId,
}: ActivityComponentProps) => {
  const metadata = message?.metadata;
  const disabled = metadata?.is_active === "False";
  const deadlineDate = metadata?.deadlineDate || "April 26";
  const deadlineTime = metadata?.deadlineTime || "4:33 PM";
      ...
      >
        ...
        <View style={styles.descriptionWrapper}>
          <Text style={styles.description}>
            {message?.content || `Thank you for buying on DigitizeApp. You have until ${deadlineDate} at ${deadlineTime} to report an issue. After that, refunds will no longer be available. How was your experience?`}
          </Text>
        </View>
...
export default OrderCompletedActivity;
